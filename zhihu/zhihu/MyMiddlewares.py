...
class RandomProxy(object):
    def __init__(self):
        self.Proxies = []
        # 读取json文件中的IP
        with open(r"D:\desktop\python actual\Proxy\proixes.json","rb") as f:
            for ip in f:
                self.Proxies.append(json.loads(ip))
    def process_request(self,request,spider):
        # 每次访问时获取一个随机的代理IP
...
class SeleniumMiddleware(object):
    def process_request(self, request, spider):
        ...
        if used_selenium:
            ...
            spider.browser.find_element_by_id("email").send_keys("18247588821")
            time.sleep(2)
            spider.browser.find_element_by_id("password").clear()
            spider.browser.find_element_by_id("password").send_keys("19960123SHUIping")
            time.sleep(2)
